...
class ErasureTokenRequest(BaseModel):
    ...
    target_path: str = Field(..., examples=["/evidence/sandbox/target_folder"])
    approved_root: str = Field(..., examples=["/evidence/sandbox"])
...
class ErasureTokenResponse(BaseModel):
    ...
    required_confirmation_string: str = Field(
        ..., examples=["CONFIRM:/evidence/sandbox/target_folder:a1b2c3d4"]
    )
...
class ErasureItemResult(BaseModel):
    ...
    path: str
    item_type: str = Field(..., examples=["FILE"], description="FILE, DIRECTORY, SYMLINK, SPECIAL")
    status: str = Field(
        ...,
        examples=["ERASED"],
    ...
    message: str = Field(..., examples=["Logical file overwrite and unlink completed."])
...
class ErasureExecuteRequest(BaseModel):
    ...
    target_path: str = Field(..., examples=["/evidence/sandbox/target_folder"])
    approved_root: Optional[str] = Field(None, examples=["/evidence/sandbox"])
    confirmation_token: Optional[str] = Field(
        None, examples=["CONFIRM:/evidence/sandbox/target_folder:a1b2c3d4"]
    )
    ...
    reason: str = Field("Standard DFIR file erasure", min_length=3, examples=["Authorized evidence directory cleanup"])
...
class ErasureReport(BaseModel):
    ...
    status: str = Field(..., examples=["COMPLETED"], description="SIMULATED, COMPLETED, PARTIAL_SUCCESS, FAILED")
